(ns tanara.views.questoes
  (:require [tanara.views.common :as common]
            [noir.content.getting-started]
            [noir.session :as session])
  (:use [tanara.models.estudante.corygil estudante]
        [tanara.models.pedagogico pedagogico]
...
(cond (= op "al1d") (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario))  "alocDin" "ex1" 1.0 0.0 0.0)
 :else (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario)) "alocDin" "ex1" 0.0 0.0 1.0))
     (common/layout
         [:body {:id "fundoiframe"} 
...
(cond (= op "al2c") (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario))  "alocDin" "ex2" 1.0 0.0 0.0)
 :else (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario)) "alocDin" "ex2" 0.0 0.0 1.0))     (common/layout
         [:body {:id "fundoiframe"} 
...
(cond (= op "al3d") (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario))  "alocDin" "ex3" 1.0 0.0 0.0)
 :else (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario)) "alocDin" "ex3" 0.0 0.0 1.0))     (common/layout
         [:body {:id "fundoiframe"} 
...
(cond (= op "al1c") (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario))  "alocDin" "ex4" 1.0 0.0 0.0)
 :else (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario)) "alocDin" "ex4" 0.0 0.0 1.0))
     (common/layout
         [:body {:id "fundoiframe"} 
...
(cond (= op "al5c") (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario))  "alocDin" "ex5" 1.0 0.0 0.0)
 :else (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario)) "alocDin" "ex5" 0.0 0.0 1.0))   (common/layout
         [:body {:id "fundoiframe"} 
...
(cond (= op "al6c") (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario))  "alocDin" "ex6" 1.0 0.0 0.0)
 :else (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario)) "alocDin" "ex6" 0.0 0.0 1.0))
     (common/layout
         [:body {:id "fundoiframe"} 
...
(cond (= op "al7b") (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario))  "alocDin" "ex7" 1.0 0.0 0.0)
 :else (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario)) "alocDin" "ex7" 0.0 0.0 1.0))
     (common/layout
         [:body {:id "fundoiframe"} 
...
(cond (= op "al8d") (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario))  "alocDin" "ex8" 1.0 0.0 0.0)
 :else (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario)) "alocDin" "ex8" 0.0 0.0 1.0))
     (common/layout
         [:body {:id "fundoiframe"} 
...
(cond (= op "al9d") (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario))  "alocDin" "ex9" 1.0 0.0 0.0)
 :else (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario)) "alocDin" "ex9" 0.0 0.0 1.0))
     (common/layout
         [:body {:id "fundoiframe"} 
...
(cond (= op "al10c") (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario))  "alocDin" "ex10" 1.0 0.0 0.0)
 :else (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario)) "alocDin" "ex10" 0.0 0.0 1.0))
...
(cond (> (get (nth (retorna-exercicio-certos-dominio (recupera-id (session/get :senhaUsuario)) "alocDin") 0) :bom) 0.5)
  (common/layout
         [:body {:id "fundoiframe" :onload "alteraVisibilidadeAlocacaoInvisivel();"} 
         [:center [:h5 "Alocação Dinâmica"]]
   ...
   :else 
     (common/layout
         ...
         [:center [:h4 [:video {:id "video" :controls "controls"}
...
(defpage [:post "/login/vetor/1"] []
    (common/layout
         ...
         [:center [:h5 "VETOR"]]
         [:p "1) Quero atribuir a uma variável X a quinta posição de um vetor V de 100 posições. Para isto:"]
         [:input {:type "radio" :name "op" :value "v1a" }]  " (A) X = V[5]" [:br]
         [:input {:type "radio" :name "op" :value "v1b" }]  " (b) X = V[4]" [:br]
         [:input {:type "radio" :name "op" :value "v1c" }]  " (C) Não é possível acessá-la diretamente, preciso fazer um laço que percorre até v[4]." [:br]
         [:input {:type "radio" :name "op" :value "v1d" :onclick "" }]  " (D) Nenhuma das alternativas anteriores." [:br] [:br][:br]
         [:button {:class "botaoQuestoes" :onclick "return verificaRadio();"} "Avançar"]]]))



(defpage [:post "/login/vetor/2"] {:keys [op]}
(cond (= op "v1b") (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario))  "vetor" "ex1" 1.0 0.0 0.0)
 :else (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario)) "vetor" "ex1" 0.0 0.0 1.0)) 
    (common/layout
         [:body {:id "fundoiframe"} 
         [:form {:action "/login/vetor/3" :method "post" :name "form"}
         [:center [:h5 "VETOR"]]
         [:p "2) A individualização de cada variável de um vetor é feita através do uso de : "]
         [:input {:type "radio" :name "op" :value "v2a" }]  " (A) dimensão." [:br]
         [:input {:type "radio" :name "op" :value "v2b" }]  " (B) coluna." [:br]
         [:input {:type "radio" :name "op" :value "v2c" }]  " (C) linha." [:br]
         [:input {:type "radio" :name "op" :value "v2d" }]  " (D) indíces." [:br] [:br][:br]
         [:button {:class "botaoQuestoes" :onclick "return verificaRadio();" } "Avançar"]]]))


(defpage [:post "/login/vetor/3"] {:keys [op]}
(cond (= op "v2d") (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario))  "vetor" "ex2" 1.0 0.0 0.0)
 :else (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario)) "vetor" "ex2" 0.0 0.0 1.0))
     (common/layout
         [:body {:id "fundoiframe"} 
         [:form {:action "/login/vetor/4" :method "post" :name "form"}
         [:center [:h5 "VETOR"]]
         [:p "3) Qual código a seguir percorre o vetor de caracteres de 4 posições, imprimindo cada item : "]
         [:input {:type "radio" :name "op" :value "v3a" }]  " (A) for(i = 0, i <= 4, i++)  printf(%d, vetor[i]);" [:br]
         [:input {:type "radio" :name "op" :value "v3b" }]  " (B) for(i = 0; i < 4, i++)  printf(%d,vetor[i]);" [:br]
         [:input {:type "radio" :name "op" :value "v3c" }]  " (C) for(i = 0; i < 4, i++)  printf(%c,vetor[i]);" [:br]
         [:input {:type "radio" :name "op" :value "v3d" }]  " (D) Nenhuma das anteriores." [:br] [:br][:br]
         [:button {:class "botaoQuestoes" :onclick "return verificaRadio();"} "Avançar"]]]))


(defpage [:post "/login/vetor/4"] {:keys [op]}
(cond (= op "v3c") (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario))  "vetor" "ex3" 1.0 0.0 0.0)
 :else (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario)) "vetor" "ex3" 0.0 0.0 1.0))
     (common/layout
         [:body {:id "fundoiframe"} 
         [:form {:action "/login/vetor/5" :method "post" :name "form"}
         [:center [:h5 "VETOR"]]
         [:p "4) Os vetores são matrizes de quantas dimensões:"]
         [:input {:type "radio" :name "op" :value "v4a" }]  " (A) 1 dimensão" [:br]
         [:input {:type "radio" :name "op" :value "v4b" }]  " (B) 0 dimensão" [:br]
         [:input {:type "radio" :name "op" :value "v4c" }]  " (C) 2 dimensões" [:br]
         [:input {:type "radio" :name "op" :value "v4d" }]  " (D) Nenhuma das anteriores." [:br] [:br][:br] 
         [:button {:class "botaoQuestoes" :onclick "return verificaRadio();"} "Avançar"]]]))


(defpage [:post "/login/vetor/5"] {:keys [op]}   
(cond (= op "v4a") (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario))  "vetor" "ex4" 1.0 0.0 0.0)
 :else (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario)) "vetor" "ex4" 0.0 0.0 1.0))
 (common/layout
         [:body {:id "fundoiframe"} 
         [:form {:action "/login/vetor/6" :method "post" :name "form"}
         [:center [:h5 "VETOR"]]
         [:p "5) O que esse código faz : for(i=0; i<5; i++) Vetor[i] = i+2 : "]
         [:input {:type "radio" :name "op" :value "v5a" }]  "Preenche o Vetor com o número 1." [:br]
         [:input {:type "radio" :name "op" :value "v5b" }]  "Preenche o Vetor de 1 à 5." [:br]
         [:input {:type "radio" :name "op" :value "v5c" }]  "Preenche o Vetor com número 0." [:br]
         [:input {:type "radio" :name "op" :value "v5d" }]  "Nenhuma das anteriores." [:br] [:br][:br]
         [:button {:class "botaoQuestoes" :onclick "return verificaRadio();"} "Avançar"]]]))

(defpage [:post "/login/vetor/6"] {:keys [op]}
(cond (= op "v5d") (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario))  "vetor" "ex5" 1.0 0.0 0.0)
 :else (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario)) "vetor" "ex5" 0.0 0.0 1.0))
     (common/layout
         [:body {:id "fundoiframe"} 
         ...
         [:input {:type "radio" :name "op" :value "v6a" }]  " (A) int A[5]" [:br]
         [:input {:type "radio" :name "op" :value "v6b" }]  " (B) float A[5]" [:br]
         [:input {:type "radio" :name "op" :value "v6c" }]  " (C) char A[5]" [:br]
         [:input {:type "radio" :name "op" :value "v6d" }]  " (D) void A[5]" [:br] [:br][:br]
         [:button {:class "botaoQuestoes" :onclick "return verificaRadio();"} "Avançar"]]]))

(defpage [:post "/login/vetor/7"] {:keys [op]}
(cond (= op "v6d") (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario))  "vetor" "ex6" 1.0 0.0 0.0)
 :else (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario)) "vetor" "ex6" 0.0 0.0 1.0))
     (common/layout
         [:body {:id "fundoiframe"} 
         [:form {:action "/login/vetor/8" :method "post" :name "form"}
         [:center [:h5 "VETOR"]]
         [:p "7) Qual função é usada para pegar o tamanho de um vetor ?"]
         [:input {:type "radio" :name "op" :value "v7a" }]  " (A) sizeof()" [:br]
         [:input {:type "radio" :name "op" :value "v7b" }]  " (B) malloc()" [:br]
         [:input {:type "radio" :name "op" :value "v7c" }]  " (C) Não é possível pegar com uma função." [:br]
         [:input {:type "radio" :name "op" :value "v7d" }]  " (D) Nenhuma das alternativas anteriores." [:br] [:br][:br]
         [:button {:class "botaoQuestoes" :onclick "return verificaRadio();"} "Avançar"]]]))

(defpage [:post "/login/vetor/8"] {:keys [op]}
(cond (= op "v7a") (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario))  "vetor" "ex7" 1.0 0.0 0.0)
 :else (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario)) "vetor" "ex7" 0.0 0.0 1.0))
     (common/layout
         [:body {:id "fundoiframe"} 
         [:form {:action "/login/vetor/9" :method "post" :name "form"}
         [:center [:h5 "VETOR"]]
         [:p "8) Dado um vetor A definido com: int A[99]; como preenchê-lo com o valor 30? "]
         [:input {:type "radio" :name "op" :value "v8a" }]  " (A) afor (int i = 0; i <= 99; i++) A[i] = 30;" [:br]
         [:input {:type "radio" :name "op" :value "v8b" }]  " (B) for  (int i = 0; i < 99; i++)  A[i] = 30;" [:br]
         [:input {:type "radio" :name "op" :value "v8c" }]  " (C) for (int i = 1; i <= 99; i++)  A[i] = 30;" [:br]
         [:input {:type "radio" :name "op" :value "v8d" }]  " (D) for (int i = 0; i <= 99; i++){
 		                                              A[i] = 30;
                                                               i++;
                                                                                                }" [:br] [:br][:br]
         [:button {:class "botaoQuestoes" :onclick "return verificaRadio();"} "Avançar"]]]))
...
(cond (= op "v8b") (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario))  "vetor" "ex8" 1.0 0.0 0.0)
 :else (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario)) "vetor" "ex8" 0.0 0.0 1.0))
     (common/layout
         [:body {:id "fundoiframe"} 
         [:form {:action "/login/vetor/10" :method "post" :name "form"}
         [:center [:h5 "VETOR"]]
         [:p "9) Dado um vetor B definido com: int B[20]; como preenchê-lo com números inteiros 1, 2, 3, ..., 20. "]
         [:input {:type "radio" :name "op" :value "v9a" }]  " (A)for (int i = 0; i <= 20; i++) A[i] = i;" [:br]
         [:input {:type "radio" :name "op" :value "v9b" }]  " (B) for (int i = 0; i < 19; i++) A[i] = i;" [:br]
         [:input {:type "radio" :name "op" :value "v9c" }]  " (C) for (int i = 1; i < 19; i++) A[i] = i;" [:br]
         [:input {:type "radio" :name "op" :value "v9d" }]  " (D) for (int i = 1; i <= 20; i++) A[i-1] = i" [:br] [:br][:br]
         [:button {:class "botaoQuestoes" :onclick "return verificaRadio();"} "Avançar"]]]))


(defpage [:post "/login/vetor/10"] {:keys [op]}
(cond (= op "v9d") (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario))  "vetor" "ex9" 1.0 0.0 0.0)
 :else (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario)) "vetor" "ex9" 0.0 0.0 1.0))
     (common/layout
         [:body {:id "fundoiframe"} 
         [:form {:action "/login/vetor/fim" :method "post" :name "form"}
         [:center [:h5 "VETOR"]]
         [:p "10) Dada uma sequência de n números, imprime-las na ordem inversa à da leitura. "]
         [:input {:type "radio" :name "op" :value "v10a" }]  " (A) for (i = 0; i < n; i++) 
                                                               scanf(%d, &v[i]);
                                                               for (i = n-1; i >= 0; i--)
                                                               printf(%d, v[i]);" [:br]
                                                                                    
         [:input {:type "radio" :name "op" :value "v10b" }]  " (B) for (i = 1; i < n; i++)
                                                                scanf(%d, &v[i]);
                                                                for (i = n-1; i >= 1; i--)
                                                                printf(%d, v[i]);" [:br]
 
         [:input {:type "radio" :name "op" :value "v10c" }]  " (C) for (i = 1; i < n; i++)
                                                                scanf(%d, &v[i]);
   								for (i = n; i >= 1; i--)
        							printf(%d, v[i]);" [:br]

         [:input {:type "radio" :name "op" :value "v10d" }]  " (D) for (i = 1; i < n; i++)
   								scanf(%d, &v[i]);
   								for (i = n; i > 1; i--)
        							printf(%d, v[i]);" [:br] [:br][:br]
         [:button {:class "botaoQuestoes" :onclick "return verificaRadio();"} "Avançar"]]]))

(defpage [:post "/login/vetor/fim"] {:keys [op]}
(cond (= op "v10a") (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario))  "vetor" "ex10" 1.0 0.0 0.0)
 :else (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario)) "vetor" "ex10" 0.0 0.0 1.0))
...
(cond (> (get (nth (retorna-exercicio-certos-dominio (recupera-id (session/get :senhaUsuario)) "vetor") 0) :bom) 0.5)
  (common/layout
         [:body {:id "fundoiframe" :onload "alteraVisibilidadeLista();"} 
         [:center [:h5 "VETOR"]]
   ...
   :else 
     (common/layout
         [:body {:id "fundoiframe" :onload "alteraVisibilidadeAlocacao();"} 
         [:center [:h5 "VETOR" ]]
         [:center [:h2 "Você terminou as atividades de vetores! "]]
...
(cond (= op "l1c") (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario))  "lista" "ex1" 1.0 0.0 0.0)
 :else (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario)) "lista" "ex1" 0.0 0.0 1.0))     (common/layout
         [:body {:id "fundoiframe"} 
...
(cond (= op "l2c") (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario))  "lista" "ex2" 1.0 0.0 0.0)
 :else (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario)) "lista" "ex2" 0.0 0.0 1.0))
     (common/layout
         [:body {:id "fundoiframe"} 
...
(cond (= op "l3d") (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario))  "lista" "ex3" 1.0 0.0 0.0)
 :else (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario)) "lista" "ex3" 0.0 0.0 1.0))
     (common/layout
         [:body {:id "fundoiframe"} 
...
(cond (= op "l5d") (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario))  "lista" "ex5" 1.0 0.0 0.0)
 :else (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario)) "lista" "ex5" 0.0 0.0 1.0))
     (common/layout
         [:body {:id "fundoiframe"} 
...
(cond (= op "l6d") (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario))  "lista" "ex6" 1.0 0.0 0.0)
 :else (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario)) "lista" "ex6" 0.0 0.0 1.0))
     (common/layout
         [:body {:id "fundoiframe"} 
...
(cond (= op "l7d") (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario))  "lista" "ex7" 1.0 0.0 0.0)
 :else (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario)) "lista" "ex7" 0.0 0.0 1.0))
     (common/layout
         [:body {:id "fundoiframe"} 
...
(cond (= op "l8b") (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario))  "lista" "ex8" 1.0 0.0 0.0)
 :else (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario)) "lista" "ex8" 0.0 0.0 1.0))
     (common/layout
         [:body {:id "fundoiframe"} 
...
(cond (= op "l9c") (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario))  "lista" "ex9" 1.0 0.0 0.0)
 :else (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario)) "lista" "ex9" 0.0 0.0 1.0))
     (common/layout
         [:body {:id "fundoiframe"} 
...
(cond (= op "l10c") (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario))  "lista" "ex10" 1.0 0.0 0.0)
 :else (atualizar-probs-exercicio (recupera-id (session/get :senhaUsuario)) "lista" "ex10" 0.0 0.0 1.0))
...
(cond (> (get (nth (retorna-exercicio-certos-dominio (recupera-id (session/get :senhaUsuario)) "lista") 0) :bom) 0.5)
  (common/layout
         [:body {:id "fundoiframe" :onload "alteraVisibilidadeFila();"} 
         [:center [:h5 "Lista"]]
...
(defpage [:post "/login/fila/fim"] []
     (common/layout
         [:body {:id "fundoiframe" :onload "alteraVisibilidadePilha();"} 
         [:center [:h5 "FILA"]]
...
(defpage [:post "/login/pilha/fim"] []
     (common/layout
         [:body {:id "fundoiframe" :onload "alteraVisibilidadeArvore();"} 
         [:center [:h5 "Pilha"]]
...
(defpage [:post "/login/arvore/fim"] []
     (common/layout
         [:body {:id "fundoiframe" :onload "alteraVisibilidadeMetOrd();"} 
         [:center [:h5 "ÁRVORE"]]
...
(defpage [:post "/login/metOrd/fim"] []
     (common/layout
         [:body {:id "fundoiframe" :onload "alteraVisibilidadeMetPesq();"} 
         [:center [:h5 "Metodos de Ordenação"]]
