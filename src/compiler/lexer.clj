...
(defn read-comment [rdr line col]
   (do #_(println "Entrando no comentario")
   (loop [c (.read rdr)
          ln line
          cl (inc col)]
     (cond (= c -1) 
           ...
           (= c (int \*))
              (let [c1 (.read rdr)
                ...
                (cond 
                      (= c1 (int \/))              ; * /
                         (do #_(println "Saindo do comentario" ln)
...
(defn read-line-comment [rdr line col]
   (do #_(println "Entrando no comentario de linha")
   ...
   (loop [c  (.read rdr)
     ...
     (cond (= c -1) 
           ...
           (#{\return \newline} (char c))  ; \n
            (do #_(println "Saindo do comentario de linha")
              ...
              (loop [c (.read rdr)
                 ...
                 (cond (Character/isDigit c) 
                         (recur (.read rdr) 
                                ...
                                (* d 10))
                       :else (list {:tag 'float-const 
                                     :value x 
                                     :start {:line line :col col}  
                                     :end   {:line ln   :col (dec cl) } } 
                                    c
                                    ln cl)))
           :else (list {:tag 'int-const 
                         :value v 
                         :start {:line line :col col}  
                         :end   {:line ln   :col (dec cl) }} 
                        c 
...
(defn read-token [rdr c line col]
  (let [[c line col] (skip-spaces-and-comment rdr c line col)] 
     (cond (= c -1) 
           ...
           (= c (int \~))
              (token-kwd 'tilde (.read rdr) 1 line (inc col))
           ...
           (= c (int \&))
              (let [ nextc (.read rdr) 
                     cl (inc col) ]
                 (cond (= nextc (int \=))             ; &=
                          (token-kwd '&= (.read rdr) 2 line (inc cl))
                       (= nextc (int \&))             ; &&
                          (token-kwd '&& (.read rdr) 2 line (inc cl))   
                       :else                          ; &
                          (token '& nextc line cl)))
           ...
           (= c (int \^))
              (let [ nextc (.read rdr) 
                     cl (inc col) ]
                 (cond (= nextc (int \=))             ; ^=
                          (token-kwd 'bxor= (.read rdr) 2 line (inc cl))
                       :else                          ; ^
                          (token (char c) nextc line cl)))
           ...
           (= c (int \/))
              (let [ nextc (.read rdr) 
                     cl (inc col) ]
                 (cond (= nextc (int \=))             ; /=
                          (token-kwd 'div= (.read rdr) 2 line (inc cl))
                       :else                          ; /
                          (token '/ nextc line cl)))
              ...
              (let [ nextc (.read rdr) 
                 ...
                 (cond (= nextc (int \>))
                          (let [nextc2 (.read rdr)
                                cl (inc cl)]
                             (if (= nextc2 (int \=))                   ; >>=
                                (token-kwd '>>= (.read rdr) 3 line (inc cl))
                                (token-kwd '>> nextc2 2 line cl))) ; >> 
                       (= nextc (int \=)) ; >=
                          (token-kwd '>= (.read rdr) 2 line (inc cl))   
                       :else                                           ; >
                          (token '> nextc line cl)))
