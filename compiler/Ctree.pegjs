...
// (6.7) 
declaration =
   spec:declaration_specifiers idl:init_declarator_list? SEMI {  
        return {
          tree: { tag: "declaration" ,
                  cld: [spec.tree, 
                        idl ? 
                        idl.tree :
                        'none'] }
...
declaration_specifiers =
   ...
   tl:( storage_class_specifier
      ...
      / function_specifier
      / alignment_specifier
      )* { var a=hd; a=a.concat(tp); a=a.concat(tl); 
           return {
             tree: {tag: "declaration_specifiers",
                    cld: a}}; 
...
/   a:( storage_class_specifier
      ...
      / alignment_specifier
          ...
          return {
             tree: { tag: "declaration_specifiers",
                     cld: a.map(function(x) { return x.tree;}) }
          };
      }
...
init_declarator =
   d:declarator init:(ATRIB i:initializer { return i; } )? {
      return {
         tree: { tag: "init_declarator",
                 cld: [d.tree, init? init.tree : 'none']
               } 
      };
   }
...
tp:(  TYPEDEF
   ...
   /  REGISTER
      ...
      return { 
        tag: tp,
        type: 'none',
        tree: { tag: "storage_class_specifier",
                cld: [tp] }
      };
}
...
struct_or_union_specifier =
   ...
   LWING sdl:struct_declaration_list RWING {
     return { tag: su,
              name: id? id : 'none',
              fields: sdl,
              tree: { tag: "struct_or_union_specifier",
                      cld: [su.tree, id, sdl.tree] }
            };
...
struct_or_union =
  su:( STRUCT
     / UNION ) {
   return { tree: { tag: "struct_or_union",
                    cld: [su] }
         };
}
...
struct_declaration_list =
   sdl:struct_declaration+ {
     return { tree: {tag: "struct_declaration_list",
                     cld: sdl.map(function(x){ return x.tree; }) }
     };
   }

// (6.7.2.1) 
struct_declaration =
   spec:specifier_qualifier_list sdl:struct_declarator_list? SEMI {
      return { tree: { tag: "struct_declaration",
                       cld: [spec.tree, sdl ? sdl.tree : 'none'] }
      };
   }   
...
specifier_qualifier_list =
   sql:(type_specifier / type_qualifier)+ {
      return { tree: { tag: "specifier_qualifier_list",
                       cld: sql.map(function(x) { return x.tree; }) }
      };
   }
...
struct_declarator_list =
   ...
   tl:(COMMA sd:struct_declarator { return sd; })* {
      return {tree: {tag: "struct_declarator_list",
                     cld: [hd.tree].concat(tl.map(function(x){ return x.tree; }))}
      };
   }
...
declarator =
   ptr:pointer? dec:direct_declarator {
     return {
       tree: { tag: "declarator",
               cld: [ptr? ptr.tree : 'none', dec.tree] }
     };
}
...
direct_declarator =
   hd:direct_declarator_head tl:direct_declarator_tail*  {               
     return {tree: {tag: "direct_declarator",
                    cld: [hd.tree].concat(tl ? 
                         tl.map(function(x){ return x.tree;}) : 
                         []) }
            };
...
direct_declarator_head =   
   id:identifier { 
     return {tree: {tag: "direct_declarator_head",
                    cld: [id] }
            };
   }
...
parameter_declaration =
   ...
   dec:( declarator / abstract_declarator? ) {
     return {
       tree: { tag: 'parameter_declaration',
               cld: [specs.tree, 
                     dec ? dec.tree : 'none'] }
     };
   }
...
initializer_list =
   ...
   tl:( COMMA designation? i:initializer { return i; })* {
     return { 
        tree: { tag: "initializer_list",
                cld: [d ? d.tree : 'none', 
                      hd.tree, 
                      tl ? tl.map(function(x){ return x.tree; }) :
                      'none'] }
     };
   }
